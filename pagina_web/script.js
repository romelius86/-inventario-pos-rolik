...
function renderInventory(products) {
    const columns = [
        ...
        { 
            key: 'nombre', 
            formatter: (val, p) => `<strong>${val}</strong><br><small class="text-muted">${p.fabricante || ''}</small>` 
        },
...
function filterInventory() {
    ...
    const filtered = allInventory.filter(p => {
        const text = (p.nombre + " " + p.codigo + " " + p.categoria).toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
